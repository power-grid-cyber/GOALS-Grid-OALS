...
==================================================
Drops in the real OpenDSS engine when opendssdirect is installed, falls back to the built-in Newton-Raphson solver automatically.
...
class OpenDSSNetworkSimulator:
    ...
    #  BUILT-IN NEWTON-RAPHSON BACKEND
    def _build_network(self):
        self.buses: Dict[str, Bus] = {
            ...
            'bus_684':  Bus('bus_684'),
            'bus_680':  Bus('bus_680',  P_gen_mw=0.60),                             # Wind with 0.60 MW capacity
            'bus_611':  Bus('bus_611',  P_mw=0.170, Q_mvar=0.080, P_gen_mw=0.25),   # Solar with 0.25MW capacity
            'bus_652':  Bus('bus_652',  P_mw=0.128, Q_mvar=0.086, P_gen_mw=0.15),   # Solar with 0.15MW capacity
        }
        self.branches: List[Branch] = [
            Branch('bus_sub',  'bus_650',  0.002,  0.008),
            Branch('bus_650',  'bus_632',  0.038,  0.111),
            ...
            Branch('bus_650',  'bus_684',  0.017,  0.049),
    ...
    def _build_ybus(self):
        ...
        for br in self.branches:
            ...
            y = 1.0 / complex(br.R_pu, br.X_pu); yb = 0.5j * br.B_pu
            self.Y[i,i]+=y+yb; self.Y[j,j]+=y+yb
            self.Y[i,j]-=y;    self.Y[j,i]-=y
    ...
    def _nr(self, Ps, Qs, maxiter=20, tol=1e-8):
        ...
        for it in range(maxiter):
            ...
            th[:npq]+=dx[:npq]
            Vm[:npq]*=np.clip(1.0+dx[npq:],0.90,1.10)
            Vm[:npq]=np.clip(Vm[:npq],0.75,1.25)
    ...
    def _update_frequency(self, P_dc_mw: float, dt: float):
        ...
        d_dev = (60.0 / (2.0 * self._H_sys)) * (
            dg_pu - self._D_sys * self._freq_dev / 60.0) * dt

        self._freq_dev = self._freq_dev * np.exp(-dt / 4.0) + d_dev
